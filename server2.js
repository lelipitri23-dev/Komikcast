...
app.get('/api/cron-update', async (req, res) => {
  // Cek otorisasi sederhana (biar gak ditembak orang iseng)
  const authHeader = req.headers.authorization;
  if (authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
    return res.status(401).json({ error: 'Unauthorized' });
  }

  console.log('⏰ [CRON TRIGGERED] Start Auto-Update...');
  try {
    // LIMITASI: Vercel punya timeout 10 detik (Hobby Plan).
    // Kita tidak bisa scrape semua komik sekaligus. 
    // Kita ambil 5 komik terlama diupdate saja.
    const mangasToUpdate = await Manga.find().sort({ lastUpdated: 1 }).limit(5);
    ...
    console.log(`✅ [CRON FINISHED] Updated ${updatedCount} mangas.`);
...
// 2. MIDDLEWARE 404 (Tetap Sama)
app.use((req, res, next) => {
  res.status(404).render('404', { url: req.originalUrl });
});


// 3. EXPORT MODULE (Wajib untuk Vercel)
// Vercel butuh module.exports = app
// Tapi kita juga butuh app.listen untuk local development
if (process.env.NODE_ENV !== 'production') {
  app.listen(PORT, () => {
    console.log(`🚀 Server berjalan (Local) di http://localhost:${PORT}`);
  });
}
